	...
	while($row_posts = mysqli_fetch_array($run_posts))
	{
		...
		if($content == "No" && strlen($upload_image) >=1)
		{
			echo "<div id='own_posts'>
				...
				<a href='single.php?post_id=$post_id' style='float:right;'><button class='btn btn-success abhi'>View</button></a><br>
		...
		else 
		{
			...
			$user = $_SESSION['user_email'];
			$get_user = "select * from users where user_email= '$user'";
			$run_user = mysqli_query($con,$run_user);
			$row = mysqli_fetch_array($run_user);
			$user_id = $row['user_id'];
			...
			else
			{
				...
				<a href='edit_post.php?post_id=$post_id' style='float:right;'><button class='btn btn-success don' type='submit'>Edit</button></a>
				<a href='functions/delete_post.php?post_id=$post_id' style='float:right;margin-top:-500px;'><button class='btn btn-danger'>Delete</button></a>
				<br><br>";
